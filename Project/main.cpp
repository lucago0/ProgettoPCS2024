...
int main()
{
    ...
    for (unsigned int id1 = 0; id1<fractures.NumberFractures; id1++) {
        for (unsigned int id2 = id1+1; id2<fractures.NumberFractures; id2++) {  //siamo sicuri che id2 debba iniziare da id+1?

            if (areClose(fractures,id1,id2)){
                Vector4d coeff1 = Piano(id1,fractures);
                Vector4d coeff2 = Piano(id2,fractures);
                Vector3d n1 = coeff1.head(3);
                Vector3d n2 = coeff2.head(3);
                if (n1.cross(n2) != Vector3d::Zero(3)){  //solo se i piani non sono paralleli ci può essere intersezione
